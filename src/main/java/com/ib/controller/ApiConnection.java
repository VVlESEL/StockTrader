...
public class ApiConnection extends EClientSocket {
	...
	public synchronized void placeOrder(Contract contract, Order order) {
		...
		// IexApiController.kt requires TWS 932 or higher; this limitation could be removed if needed
		if (serverVersion() < 66) {
            error( EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "IexApiController.kt requires TWS build 932 or higher to place orders.");
            return;
		}
